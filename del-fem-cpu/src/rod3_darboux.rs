...
pub fn add_gradient_of_dot_frame_axis(
    ...
    {
        let tmp0 = frmb[i1_axis].mult_mat3_array_of_array(&dfadp[i0_axis]).scale(c);
        dvdp[0].sub_in_place(&tmp0);
        dvdp[1].add_in_place(&tmp0);
    }
    {
        let tmp0 = frma[i0_axis].mult_mat3_array_of_array(&dfbdp[i1_axis]).scale(c);
    ...
    for iaxis in 0..3 {
        ...
        let mut dudt = [0.0, 0.0];
        {
            add_gradient_of_dot_frame_axis(
                &mut dudp, &mut dudt,
                1.,
                jaxis, &frma, &dfadp, &dfadt,
                kaxis, &frmb, &dfbdp, &dfbdt);
            add_gradient_of_dot_frame_axis(
                &mut dudp, &mut dudt,
                -1.,
                kaxis, &frma, &dfadp, &dfadt,
                jaxis, &frmb, &dfbdp, &dfbdt);
        }
        c[iaxis] = u / s;
        {
            let t0 = 1.0 / s;
