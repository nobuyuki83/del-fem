...
pub fn wdw<Real>(
    p_ini: &[[Real; 2]; 3],
    p_def: &[[Real; 3]; 3]) -> ([Real;3], [[[Real;3];3];3])
where Real: num_traits::Float
{
    ...
    let gd1_x = [p_def[1][0] - p_def[0][0], p_def[1][1] - p_def[0][1], p_def[1][2] - p_def[0][2]];
    let gd1_y = [p_def[2][0] - p_def[0][0], p_def[2][1] - p_def[0][1], p_def[2][2] - p_def[0][2]];

    let glstrain = [  // green lagrange strain (with engineer's notation)
    ...
    let gugu0_xx = [ gu0_x[0] * gu0_x[0], gu0_y[0] * gu0_y[0], gu0_x[0] * gu0_y[0] ];
    let gugu0_yy = [ gu0_x[1] * gu0_x[1], gu0_y[1] * gu0_y[1], gu0_x[1] * gu0_y[1] ];
    let gugu0_xy = [ two * gu0_x[0] * gu0_x[1], two * gu0_y[0] * gu0_y[1], gu0_x[0] * gu0_y[1] + gu0_x[1] * gu0_y[0] ];
    let mut w = [zero; 3];
    ...
    hoge(&mut dw[0][0], -(gugu0_xx[0] + gugu0_xx[2]), - (gugu0_xx[1] + gugu0_xx[2]));
    hoge(&mut dw[0][1], gugu0_xx[0], gugu0_xx[2]);
    hoge(&mut dw[0][2], gugu0_xx[2], gugu0_xx[1]);
    hoge(&mut dw[1][0], -(gugu0_yy[0] + gugu0_yy[2]), - (gugu0_yy[1] + gugu0_yy[2]));
    hoge(&mut dw[1][1], gugu0_yy[0], gugu0_yy[2]);
    hoge(&mut dw[1][2], gugu0_yy[2], gugu0_yy[1]);
    hoge(&mut dw[2][0], -(gugu0_xy[0] + gugu0_xy[2]), - (gugu0_xy[1] + gugu0_xy[2]));
    ...
    T: num_traits::Float + std::ops::MulAssign + std::ops::AddAssign,
{
    ...
    let (gd0, area0) = {
        ...
        (
            [
                [p0[1][0] - p0[0][0], p0[1][1] - p0[0][1], p0[1][2] - p0[0][2]],
                [p0[2][0] - p0[0][0], p0[2][1] - p0[0][1], p0[2][2] - p0[0][2]],
                gdz,
            ],
            area0,
        )
    ...
    let gd: [[T; 3]; 2] = [
        ...
        [p1[1][0] - p1[0][0], p1[1][1] - p1[0][1], p1[1][2] - p1[0][2]],
        [p1[2][0] - p1[0][0], p1[2][1] - p1[0][1], p1[2][2] - p1[0][2]],
    ];
    ...
    let spkstress: [T; 3] = [
        ...
        elasticity_tensor[0][0] * glstrain[0] + elasticity_tensor[0][1] * glstrain[1] + elasticity_tensor[0][2] * glstrain[2],
        elasticity_tensor[1][0] * glstrain[0] + elasticity_tensor[1][1] * glstrain[1] + elasticity_tensor[1][2] * glstrain[2],
        elasticity_tensor[2][0] * glstrain[0] + elasticity_tensor[2][1] * glstrain[1] + elasticity_tensor[2][2] * glstrain[2],
    ];

    // compute energy
    let w = half * area0 * (glstrain[0] * spkstress[0] + glstrain[1] * spkstress[1] + glstrain[2] * spkstress[2]);
    ...
    for (ino, jno) in itertools::iproduct!(0..3, 0..3) {
        for (idim, jdim) in itertools::iproduct!(0..3, 0..3) {
            ...
            dtmp0 += gd[0][idim] * dNdr[ino][0] * elasticity_tensor[0][0] * gd[0][jdim] * dNdr[jno][0];
            dtmp0 += gd[0][idim] * dNdr[ino][0] * elasticity_tensor[0][1] * gd[1][jdim] * dNdr[jno][1];
            dtmp0 += gd[0][idim] * dNdr[ino][0] * elasticity_tensor[0][2] * gd[0][jdim] * dNdr[jno][1];
            dtmp0 += gd[0][idim] * dNdr[ino][0] * elasticity_tensor[0][2] * gd[1][jdim] * dNdr[jno][0];
            dtmp0 += gd[1][idim] * dNdr[ino][1] * elasticity_tensor[1][0] * gd[0][jdim] * dNdr[jno][0];
            dtmp0 += gd[1][idim] * dNdr[ino][1] * elasticity_tensor[1][1] * gd[1][jdim] * dNdr[jno][1];
            dtmp0 += gd[1][idim] * dNdr[ino][1] * elasticity_tensor[1][2] * gd[0][jdim] * dNdr[jno][1];
            dtmp0 += gd[1][idim] * dNdr[ino][1] * elasticity_tensor[1][2] * gd[1][jdim] * dNdr[jno][0];
            dtmp0 += gd[0][idim] * dNdr[ino][1] * elasticity_tensor[2][0] * gd[0][jdim] * dNdr[jno][0];
            dtmp0 += gd[0][idim] * dNdr[ino][1] * elasticity_tensor[2][1] * gd[1][jdim] * dNdr[jno][1];
            dtmp0 += gd[0][idim] * dNdr[ino][1] * elasticity_tensor[2][2] * gd[0][jdim] * dNdr[jno][1];
            dtmp0 += gd[0][idim] * dNdr[ino][1] * elasticity_tensor[2][2] * gd[1][jdim] * dNdr[jno][0];
            dtmp0 += gd[1][idim] * dNdr[ino][0] * elasticity_tensor[2][0] * gd[0][jdim] * dNdr[jno][0];
            dtmp0 += gd[1][idim] * dNdr[ino][0] * elasticity_tensor[2][1] * gd[1][jdim] * dNdr[jno][1];
            dtmp0 += gd[1][idim] * dNdr[ino][0] * elasticity_tensor[2][2] * gd[0][jdim] * dNdr[jno][1];
            dtmp0 += gd[1][idim] * dNdr[ino][0] * elasticity_tensor[2][2] * gd[1][jdim] * dNdr[jno][0];
