...
pub fn w_dw_ddw_plate_bending<T>(
    ...
    thk: T,
    lambda: T,
    myu: T) -> (T, [[T; 3]; 3], [[[T; 9]; 3]; 3])
    where T: num_traits::Float + std::ops::MulAssign + std::ops::AddAssign + 'static + Copy,
          f64: AsPrimitive<T>
{
    let zero = T::zero();
    let two = T::one() + T::one();
    ...
    {   // integrate transverse sheer stress
        let coeffa: [[T; 3]; 3] = [ // {rr,ss,sr} x {rr,ss,sr}
            [
        ...
        let coeffb: [[T; 3]; 3] = [ // {rr,ss,sr} x {rr,ss,sr}
        ...
        let de_rr_dt = (gd[0][0] * (u[1][2] - u[0][2]) - gd[0][1] * (u[1][1] - u[0][1])) * half * thk;
        let de_ss_dt = (gd[1][0] * (u[2][2] - u[0][2]) - gd[1][1] * (u[2][1] - u[0][1])) * half * thk;
        let de_rs_dt = (gd[0][0] * (u[2][2] - u[0][2]) - gd[0][1] * (u[2][1] - u[0][1])
            + gd[1][0] * (u[1][2] - u[0][2]) - gd[1][1] * (u[1][1] - u[0][1])) * onefourth * thk;
        ////
        for i_integration in 0..2 { // integration of energy related to transverse shear strain
            ...
            w += w_integration * half * (strain[0] * stress[0] + strain[1] * stress[1] + strain[2] * stress[2]);
            ...
            for (ino, jno, idof, jdof) in itertools::iproduct!(
                0..3, 0..3, 0..3, 0..3) {
                let dtmp =
                    dstrain[0][ino][idof] * coeffb[0][0] * dstrain[0][jno][jdof]
    ...
    {
        let coeffa: [[T; 2]; 2] = [ // {rt,st} x {rt,st}
            [
        ...
        let e_rt_01 = half * thk * (u[1][0] - u[0][0] + half * gd[0][0] * (u[0][2] + u[1][2]) - half * gd[0][1] * (u[0][1] + u[1][1]));
        let e_rt_12 = half * thk * (u[1][0] - u[0][0] + half * gd[0][0] * (u[1][2] + u[2][2]) - half * gd[0][1] * (u[1][1] + u[2][1]));
        let e_st_12 = half * thk * (u[2][0] - u[0][0] + half * gd[1][0] * (u[1][2] + u[2][2]) - half * gd[1][1] * (u[1][1] + u[2][1]));
        let e_st_20 = half * thk * (u[2][0] - u[0][0] + half * gd[1][0] * (u[2][2] + u[0][2]) - half * gd[1][1] * (u[2][1] + u[0][1]));
        let de_rt_01: [[T; 3]; 3] = [
        ...
        for (r, s) in pos_integration {
            ...
            for (ino, idof) in itertools::iproduct!(0..3, 0..3) {
                dw[ino][idof] += w_integration * (stress[0] * dstrain[0][ino][idof] + stress[1] * dstrain[1][ino][idof]);
            }
            for (ino, jno, idof, jdof) in itertools::iproduct!(
                0..3, 0..3, 0..3, 0..3) {
...
fn test_w_dw_ddw_plate_bending() {
    ...
    let eps = 1.0e-5;
// -------------------
    let (w0, dw0, ddw0) = w_dw_ddw_plate_bending(
        &p, &u0,
    ...
    for (ino, idof) in itertools::iproduct!(0..3, 0..3) {
        ...
        let (w1, dw1, _ddw1) = w_dw_ddw_plate_bending(
            &p, &u1,
            thickness1, lambda1, myu1);
        {
            let v0 = (w1-w0)/eps;
