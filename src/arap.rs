...
fn test_optimal_rotation_for_arap() {
    let (tri2vtx, vtx2xyz_ini)
        = del_msh::trimesh3_primitive::capsule_yup(
        0.2, 1.6, 24, 4, 24);
    let num_vtx = vtx2xyz_ini.len() / 3;
    let (row2idx, idx2col)
        = del_msh::vtx2vtx::from_uniform_mesh(&tri2vtx, 3, num_vtx, false);
...
fn test_energy_arap_spoke() {
    ...
    del_msh::io_obj::save_tri_mesh("target/hoge.obj", &tri2vtx, &vtx2xyz_def);
    for i_vtx in 0..num_vtx {
